...
from utils.logger import main_logger # Assuming trading_logger is aliased or main_logger is preferred
...
class RsiMacdEmaStrategy(TradingStrategy):
    ...
    def get_signal(self, candlesticks_df: pd.DataFrame, current_price: float, profile: Dict[str, Any]) -> str:
        ...
        log_message_parts = [f"StrategyCheck ({symbol}): Price={current_price:.6f}"]
        ...
        if 'close' not in candlesticks_df.columns:
            main_logger.error(f"RsiMacdEmaStrategy ({symbol}): DataFrame is missing 'close' prices column.")
            return "HOLD"

        # Determine minimum required data length based on active indicators
        min_len_rsi = rsi_period + 1 if use_rsi else 0 # +1 for potential previous_rsi logic if added
        min_len_macd = macd_slow_period + macd_signal_period if use_macd else 0 # MACD needs more data due to smoothing
        min_len_ema = ema_period if use_ema else 0
        required_data_length = max(min_len_rsi, min_len_macd, min_len_ema, 1) # At least 1 to prevent issues with max([])
        ...
        try:
            if use_rsi:
                ...
                if not rsi_series.empty: last_rsi = rsi_series.iloc[-1]
                log_message_parts.append(f"RSI({rsi_period})={last_rsi if last_rsi is not None else 'N/A':.2f}")


            if use_macd:     
                macd_line, signal_line, _ = calculate_macd(close_prices, macd_fast_period, macd_slow_period, macd_signal_period)
                if not macd_line.empty: last_macd = macd_line.iloc[-1]
                if not signal_line.empty: last_macd_signal = signal_line.iloc[-1]
            ...
            if use_ema:
                ...
                if not ema_series.empty: last_ema = ema_series.iloc[-1]
            ...
            main_logger.error(f"RsiMacdEmaStrategy ({symbol}): Error calculating indicators: {e}")
            return "HOLD"
        except Exception as e: # Catch any other unexpected error during calculation
            main_logger.error(f"RsiMacdEmaStrategy ({symbol}): Unexpected error during indicator calculation: {e}", exc_info=True)
            return "HOLD"
        ...
        conditions_checked = 0 # To ensure we only make a decision if at least one indicator is active
        ...
        if use_ema and last_ema is not None:
            ...
            if current_price > lower_ema_buffer_price: # Price is above the lower band (bullish sign)
                buy_conditions_met += 1
            if current_price < upper_ema_buffer_price: # Price is below the upper band (bearish sign)
                ...
                pass # The buy_conditions_met and sell_conditions_met are incremented separately.
        ...
        if use_rsi and last_rsi is not None:
            if last_rsi < rsi_oversold: buy_signal_rsi = True
            if last_rsi > rsi_overbought: sell_signal_rsi = True # Note: RSI overbought itself is not a sell signal without price confirmation or divergence

        if use_macd and last_macd is not None: # Assuming your logic for MACD buy/sell was just positive/negative
            if last_macd > 0: buy_signal_macd = True
            if last_macd < 0: sell_signal_macd = True # (and last_macd < last_macd_signal) if using crossover for sell

        if use_ema and last_ema is not None:
            # Your logic: price > ema_low_buffer for buy, price < ema_high_buffer for sell
            if current_price > (last_ema * (1 - ema_buy_buffer_pct)): buy_signal_ema = True
            if current_price < (last_ema * (1 + ema_sell_buffer_pct)): sell_signal_ema = True # This sell condition is broad

        # Determine how many indicators are configured to be used
        active_indicator_count = sum([1 for used_flag in [use_rsi, use_macd, use_ema] if used_flag])

        if active_indicator_count == 0:
            main_logger.warning(f"RsiMacdEmaStrategy ({symbol}): No indicators enabled in profile. Holding.")
            return "HOLD"
        ...
        all_buy = True
        if use_rsi and not buy_signal_rsi: all_buy = False
        if use_macd and not buy_signal_macd: all_buy = False
        if use_ema and not buy_signal_ema: all_buy = False
        
        if all_buy:
            main_logger.info(f"RsiMacdEmaStrategy ({symbol}): BUY signal generated (all active indicators confirm).")
            return "BUY"

        # SELL logic: All active indicators must give a sell signal
        all_sell = True
        if use_rsi and not sell_signal_rsi: all_sell = False
        if use_macd and not sell_signal_macd: all_sell = False
        if use_ema and not sell_signal_ema: all_sell = False # Check if this condition makes sense for your EMA sell

        if all_sell:
            main_logger.info(f"RsiMacdEmaStrategy ({symbol}): SELL signal generated (all active indicators confirm).")
            return "SELL"
...
def get_strategy_executor(strategy_name: str) -> TradingStrategy:
    ...
    if not strategy_instance:
        main_logger.error(f"Strategy '{strategy_name}' not found in strategy executor mapping.")
        available_keys = list(strategies.keys())
        raise ValueError(f"Strategy '{strategy_name}' not found. Available strategies: {available_keys}")
    
    main_logger.info(f"Successfully loaded strategy executor for: {strategy_name}")
