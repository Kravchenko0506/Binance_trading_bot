...
def get_initial_ohlcv(symbol: str, timeframe: str, limit: int) -> np.ndarray:
    ...
    trading_logger.info(
        f"TradeLogic ({symbol}): Запрос начальных {limit} OHLCV данных для {symbol} ({timeframe})...")
    try:
        ...
        if not klines:
            trading_logger.warning(
                f"TradeLogic ({symbol}): Получен пустой список klines от API.")
            return np.array([])

        close_prices = np.array([float(kline[4]) for kline in klines])
        trading_logger.info(
            f"TradeLogic ({symbol}): Успешно загружено {len(close_prices)} цен закрытия.")
        return close_prices
    except Exception as e:
        trading_logger.error(
            f"TradeLogic ({symbol}): Ошибка при получении OHLCV: {e}", exc_info=True)
        return np.array([])
...
def check_buy_sell_signals(profile: object, historic_prices_np_array: np.ndarray, current_close_price: float) -> str:
    ...
    ema_buffer = float(getattr(profile, "EMA_BUFFER", 0.002))
    ...
    try:
        ...
        if use_macd:
            macd_line, macd_signal_line = calculate_macd(
                prices_for_indicators, macd_fast_period, macd_slow_period, macd_signal_period)
        ...
        trading_logger.error(
            f"Signal Check ({symbol}): Error calculating indicators: {e}")
        return 'hold'
    except Exception as e:
        trading_logger.error(
            f"Signal Check ({symbol}): Unexpected error during indicator calculation: {e}", exc_info=True)
        return 'hold'

    # Получаем последние значения индикаторов
    last_rsi = rsi_values[-1] if use_rsi and rsi_values.size > 0 and not np.isnan(
        rsi_values[-1]) else None
    last_macd = macd_line[-1] if use_macd and macd_line.size > 0 and not np.isnan(
        macd_line[-1]) else None
    last_macd_signal = macd_signal_line[-1] if use_macd and macd_signal_line.size > 0 and not np.isnan(
        macd_signal_line[-1]) else None
    last_ema = ema_values[-1] if use_ema and ema_values.size > 0 and not np.isnan(
    ...
    if use_ema and last_ema is not None:
        if buy_signal_triggered and current_close_price < last_ema * (1 - ema_buffer):
            trading_logger.info(
            f"Signal Check ({symbol}): BUY IGNORED by EMA filter. Price {current_close_price:.6f} < EMA({ema_period}) {last_ema:.6f} - buffer {ema_buffer*100:.2f}%"
            )
            buy_signal_triggered = False

        if sell_signal_triggered and current_close_price > last_ema * (1 + ema_buffer):
            trading_logger.info(
            f"Signal Check ({symbol}): SELL IGNORED by EMA filter. Price {current_close_price:.6f} > EMA({ema_period}) {last_ema:.6f} + buffer {ema_buffer*100:.2f}%"
        )
        sell_signal_triggered = False

    # --- Формирование и логирование итогового сообщения и сигнала ---
    log_message_parts = [
        f"Signal Eval ({symbol}): Price={current_close_price:.6f}"]
    if use_rsi:
        rsi_val_str = f"{last_rsi:.2f}" if last_rsi is not None else "N/A"
        log_message_parts.append(
            f"RSI({rsi_period})={rsi_val_str} (OB:{rsi_overbought},OS:{rsi_oversold})")
    if use_macd:
